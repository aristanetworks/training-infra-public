...
class CaptureManager:
    ...
    def _parse_bridge_name(self, bridge_name: str) -> Dict:
        ...
        Bridge format: {dev1-short}{port1}-{dev2-short}{port2}
        Example: sp1Et1-le1Et1 -> spine1:Ethernet1 <-> leaf1:Ethernet1

        Short codes are generated by parseNames in kvm-topo-builder.py:
        - First 2 letters of device name + numbers
        - Ethernet# becomes Et#
        """
        result = {
            "source_device": "",
            "source_port": "",
            "source_device_name": "",  # Full name if we can resolve it
            "source_port_name": "",
            "target_device": "",
            "target_port": "",
            "target_device_name": "",
        ...
        try:
            ...
            if len(parts) >= 2:
                # Parse source: "sp1Et1" -> device="sp1", port="Et1"
                src = parts[0]
                tgt = '-'.join(parts[1:])  # Handle names with multiple dashes

                # Find where port starts (Et for Ethernet, Ma for Management)
                src_match = re.match(r'^([a-zA-Z]{2}\d+)([A-Z][a-z]?\d+)$', src)
                if src_match:
                    result["source_device"] = src_match.group(1)
                    result["source_port"] = src_match.group(2)
                    result["source_port_name"] = self._expand_port_name(src_match.group(2))
                    result["source_device_name"] = self._expand_device_name(src_match.group(1))

                tgt_match = re.match(r'^([a-zA-Z]{2}\d+)([A-Z][a-z]?\d+)$', tgt)
                if tgt_match:
                    result["target_device"] = tgt_match.group(1)
                    result["target_port"] = tgt_match.group(2)
                    result["target_port_name"] = self._expand_port_name(tgt_match.group(2))
                    result["target_device_name"] = self._expand_device_name(tgt_match.group(1))
    ...
    def _expand_port_name(self, short: str) -> str:
        """Expand port short code to full name."""
        # Et1 -> Ethernet1, Ma1 -> Management1
        match = re.match(r'^(Et|Ma)(\d+)$', short)
        if match:
            prefix = 'Ethernet' if match.group(1) == 'Et' else 'Management'
            return f"{prefix}{match.group(2)}"
        return short

    def _expand_device_name(self, short: str) -> str:
        """
        Expand device short code to likely full name.
        This is best-effort since we don't have the topology data here.
        """
        # Common patterns: sp1 -> spine1, le1 -> leaf1, ho1 -> host1
        patterns = {
            'sp': 'spine',
            'le': 'leaf',
            'ho': 'host',
        ...
        match = re.match(r'^([a-zA-Z]{2})(\d+)(.*)$', short)
        if match:
            prefix = patterns.get(match.group(1).lower(), match.group(1))
            suffix = match.group(3)  # For dc codes like 'd1'
